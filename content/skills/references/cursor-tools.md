...
| **Invoke a skill or command** | SKILL.md (skill format) | `.cursor/skills/` directory (project-local) | Cursor recognizes SKILL.md files in `.cursor/skills/` using the same format as Claude Code. Can be referenced in prompts or invoked through Agent mode. Cursor converged on the SKILL.md specification for interoperability. |
...
| **Run shell commands** | Terminal integration / Agent spawning | `.cursor/terminal` (if configured) | Agent mode can execute shell commands as part of autonomous work. Terminal output feeds back into the Agent loop. No explicit background task primitive — all execution is synchronous within the Agent thread. |
...
Place SKILL.md files in `.cursor/skills/` and reference them in Agent mode:
```
...
- **No native todo tracking:** Use code comments or external markdown notes instead.
